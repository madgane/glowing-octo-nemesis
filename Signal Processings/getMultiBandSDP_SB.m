...
if isfield(SimParams.Debug,'MultiCastSDPExchange')
    ...
    for iBase = 1:nBases
        for iBand = 1:nBands
            enabledAntenna{iBase,iBand} = SimParams.Debug.MultiCastSDPExchange;
        end
    end
    reqSINRPerUser = SimParams.Debug.SDP_ReqSINR_Band;
else
    enabledAntenna = cell(nBases,nBands);
    for iBase = 1:nBases
        for iBand = 1:nBands
    ...
    for iBase = 1:nBases
        for iGroup = 1:nGroupsPerCell(iBase,1)
            ...
            for iUser = 1:length(groupUsers)
                ...
                tempSum = -SimParams.N * reqSINRPerUser(cUser,iBand);
                for jBase = 1:nBases
                    for jGroup = 1:nGroupsPerCell(jBase,1)
                        ...
                        if and((iBase == jBase),(iGroup == jGroup))
                            ...
                            tempSum = tempSum - reqSINRPerUser(cUser,iBand) * trace(Hsdp * X{jBase,iBand}(:,:,jGroup));
                        end
                    end
                end
                    ...
                    for jBase = 1:nBases
                        for jGroup = 1:nGroupsPerCell(jBase,1)
                            ...
                            if and((iBase == jBase),(iGroup == jGroup))
                                ...
                                tempSum = tempSum - reqSINRPerUser(cUser,iBand) * grpPwr{jBase,1}(jGroup,iBand) * abs(Hsdp * Y{jBase,iBand}(:,jGroup))^2;
                            end
                        end
                    end
                    gConstraints = [gConstraints, tempSum >= 0];
