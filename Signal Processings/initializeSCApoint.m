        ...
        case 'BF'
            for iBand = 1:nBands
                for iUser = 1:kUsers
                    ...
                    M0{iBand,1}(:,:,iUser) = V(:,1:SimParams.maxRank);
                end
            end
            
        case 'Ones'
            for iBand = 1:nBands
                M0{iBand,1} = complex(ones(SimParams.nTxAntenna,SimParams.maxRank,kUsers),ones(SimParams.nTxAntenna,SimParams.maxRank,kUsers));
            end
            
        case 'Last'
            for iBand = 1:nBands
                M0{iBand,1} = SimParams.Debug.globalExchangeInfo.P{bsIndex,iBand};
                for iUser = 1:kUsers
                    cUser = linkedUsers(iUser,1);
    ...
    for iBand = 1:nBands
        for iUser = 1:kUsers
            ...
            for iLayer = 1:SimParams.maxRank
                ...
                for jUser = 1:kUsers
                    if iUser ~= jUser
                        B0(iLayer,iUser,iBand) = B0(iLayer,iUser,iBand) + norm(W0{cUser,iBand}(:,iLayer)' * cH{bsIndex,iBand}(:,:,cUser) * M0{iBand,1}(:,:,jUser),2)^2;
                    else
                        B0(iLayer,iUser,iBand) = B0(iLayer,iUser,iBand) + norm(W0{cUser,iBand}(:,iLayer)' * cH{bsIndex,iBand}(:,:,cUser) * M0{iBand,1}(:,iLayer~=1:SimParams.maxRank,iUser),2)^2;
                    end
                end
                G0(iLayer,iUser,iBand) = norm(W0{cUser,iBand}(:,iLayer)' * cH{bsIndex,iBand}(:,:,cUser) * M0{iBand,1}(:,iLayer,iUser),2)^2;
            end
        end
    end
