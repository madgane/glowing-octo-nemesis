...
switch selectionMethod
				...
				for iBand = 1:nBands
					for iUser = 1:nUsers
						for iRank = 1:maxRank
							...
							if QueuedPkts(iUser,1) < sum(tempT(:))
	...
	case 'KKTMethod-1'
		...
		phi = ones(maxRank,nUsers,nBands);
		gammaLKN = ones(maxRank,nUsers,nBands);
		
		for iUser = 1:nUsers
			for iBand = 1:nBands
				xM{iUser,iBand} = randn(SimParams.nTxAntenna,maxRank);
				...
				for iBand = 1:nBands
					for iBase = 1:nBases
						for iUser = 1:usersPerCell(iBase,1)
							...
							for iLayer = 1:maxRank
								X = SimParams.N * eye(SimParams.nRxAntenna);
								Y = SimParams.N * eye(SimParams.nRxAntenna);
								for jBase = 1:nBases
									for jUser = 1:usersPerCell(jBase,1)
										rUser = cellUserIndices{jBase,1}(jUser,1);
										if rUser ~= cUser
											H = cH{jBase,iBand}(:,:,cUser);
											X = X + H * M{rUser,iBand} * M{rUser,iBand}' * H';
											Y = Y + H * xM{rUser,iBand} * xM{rUser,iBand}' * H';
										end
									end
								end
								H = cH{iBase,iBand}(:,:,cUser);
								betaLKN(iLayer,cUser,iBand) = real(trace(X));
								gammaLKN(iLayer,cUser,iBand) = norm(H * xM{cUser,iBand}(:,iLayer))^2 / real(trace(Y));
								gammaLKN(iLayer,cUser,iBand) = gammaLKN(iLayer,cUser,iBand) + 2 * xM{cUser,iBand}(:,iLayer)' * H' * H * (M{cUser,iBand}(:,iLayer) - xM{cUser,iBand}(:,iLayer)) / real(trace(Y));								
							end
						end
					end
				end
				
				gammaLKN = max(real(gammaLKN),0);
				
				for iBand = 1:nBands
					for iUser = 1:nUsers
						for iRank = 1:maxRank
							if gammaLKN(iRank,iUser,iBand) ~= 0
								alphaLKN_1(iRank,iUser,iBand) = phi(iRank,iUser,iBand) / ((1 + gammaLKN(iRank,iUser,iBand)) * gammaLKN(iRank,iUser,iBand));
							else
								alphaLKN_1(iRank,iUser,iBand) = 0;
							end
							alphaLKN_2(iRank,iUser,iBand) = betaLKN(iRank,iUser,iBand) * phi(iRank,iUser,iBand);
						end
					end
				end
